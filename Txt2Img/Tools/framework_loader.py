...
def get_skill_data_dir() -> str:
    """Get skill data directory relative to PAI_DIR"""
    pai_dir = os.getenv('PAI_DIR', os.path.expanduser('~/.claude'))
    return os.path.join(pai_dir, 'skills/Txt2Img/Data')
...
class FrameworkDrivenGenerator:
    """Framework-driven generator"""

    def __init__(self, db_path: str = None, framework_path: str = None):
        """
        Initialize

        Args:
            db_path: Database path (defaults to PAI_DIR/skills/Txt2Img/Data/elements.db)
            framework_path: Framework config path (defaults to PAI_DIR/skills/Txt2Img/Data/prompt_framework.yaml)
        """
        data_dir = get_skill_data_dir()

        if db_path is None:
            db_path = os.path.join(data_dir, 'elements.db')
        if framework_path is None:
            framework_path = os.path.join(data_dir, 'prompt_framework.yaml')

        # Load framework
        self.framework = FrameworkLoader.load(framework_path)

        # Load IntelligentGenerator (for database queries)
