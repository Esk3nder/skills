...
describe("TestDamageControl.ts CLI", () => {
  ...
  const patternsSource = join(process.cwd(), "skills", "DamageControl", "patterns.yaml");

  beforeEach(() => {
    mkdirSync(join(testDir, "skills", "DamageControl"), { recursive: true });
    // Copy patterns.yaml so hooks can find it
    copyFileSync(patternsSource, join(testDir, "skills", "DamageControl", "patterns.yaml"));
  });
