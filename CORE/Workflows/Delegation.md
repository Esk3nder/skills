...
**ALL agents MUST end their response with a completion marker for hook capture:**

```
🗣️ [AgentName]: [Brief completion summary - what was accomplished]
```

**Examples:**
- `🗣️ Explore: Found 15 files matching the authentication pattern.`
- `🗣️ Intern: Completed research on 5 competitor companies.`
- `🗣️ Architect: Designed the microservices architecture with 4 domains.`

**Why:** The SubagentStop hook captures agent completions for MEMORY/RESEARCH/ and voice notifications. Without this marker, the hook cannot identify the completion message.
...
5. **OUTPUT FORMAT** - Include the completion marker instruction
...
Task({
  ...
  prompt: `
    ...
    ## Output Format
    End your response with a completion marker:
    🗣️ Pentester: [Brief summary of findings]
  `,
