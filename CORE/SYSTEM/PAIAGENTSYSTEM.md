...
## Agent Completion Marker (CRITICAL)

**ALL agents MUST end their response with a completion marker:**

```
🗣️ [AgentType]: [Brief summary of what was accomplished]
```

**Why:** The SubagentStop hook (`hooks/AgentOutputCapture.hook.ts`) captures agent completions for:
- Persisting to `MEMORY/RESEARCH/`
- Voice notifications for background agents
- Observability dashboard events

**Without this marker, the hook cannot identify the completion message.**

### Include in ALL Agent Prompts

```markdown
## Output Format
End your response with a completion marker:
🗣️ [YourAgentType]: [Brief summary of what you accomplished]

Example: 🗣️ Explore: Found 15 authentication-related files in src/auth/.
```

### Examples by Agent Type

| Agent Type | Example Completion |
|------------|-------------------|
| Explore | `🗣️ Explore: Mapped codebase structure with 47 TypeScript files.` |
| Intern | `🗣️ Intern: Researched 5 companies and compiled competitive analysis.` |
| Architect | `🗣️ Architect: Designed event-driven architecture with 3 bounded contexts.` |
| Engineer | `🗣️ Engineer: Implemented JWT authentication with refresh token rotation.` |
| Pentester | `🗣️ Pentester: Identified 3 SQL injection vulnerabilities in user endpoints.` |
