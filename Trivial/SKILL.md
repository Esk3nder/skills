...
|----------|---------|------|
| **PassThrough** | Any trivial prompt | `Workflows/PassThrough.md` |
...
```
User: "hi"
-> Routed to Trivial (greeting, no methodology needed)
```

**Example 2: Confirmation**
```
User: "yes continue"
-> Routed to Trivial (simple confirmation)
```

**Example 3: Low confidence match**
```
User: "what time is it"
-> Routed to Trivial (no skill match above threshold)
```

## Related Skills

**Contrast with:**
- **SystematicDebugging** - For bug fixes (not trivial)
- **Brainstorming** - For feature requests (not trivial)
- **AskDaniel** - For PAI features (not trivial)
